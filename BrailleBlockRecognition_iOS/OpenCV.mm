...
- (NSArray *) reader:(UIImage *)img{
    ...
    tr.resize(20);
    tr.clear();
    std::vector<std::vector<cv::Point> > Tr;// 三角形　エリア 座標 // = vector<Point> tr[10];
    Tr.resize(20);
    Tr.clear();
    std::vector<std::vector<cv::Point> > Sq;// ４角形　エリア 座標 Canny
    Sq.resize(20);
    Sq.clear();
    std::vector<std::vector<cv::Point> > Sqa;// ４角形　エリア 座標 Adaptive
    Sqa.resize(20);
    Sqa.clear();
    std::vector<std::vector<cv::Point> > sq;// ４角形　エリア 座標 duplicate check
    sq.resize(40);
    sq.clear();
    ...
    */
    ...
    cv::Mat img1 = cv::Mat(image.size(), CV_8UC3, cv::Scalar(100));// 背景 灰色　３角形ではよい
    cv::Mat img2 = cv::Mat(image.size(), CV_8UC3, cv::Scalar(0));// 背景　黒　for OTSU Sq find
    cv::Mat img3 = cv::Mat(image.size(), CV_8UC3, cv::Scalar(0));// 背景　黒
    cv::Mat img4 = cv::Mat(image.size(), CV_8UC3, cv::Scalar(255));//背景　白
    
    sqaindex = mask(image, img1, img2, Sqa);
    sindex = findSq(image, img2, Sq);
...
static int Trcheck(vector<vector<cv::Point> >& tr, int tindex, vector<vector<cv::Point> >& Tr )
{
    ...
    //int sx[3],sy[3];
    // 一番目
    //          Tr[0].push_back(cv::Point(tr[0][0].x, tr[0][0].y));
    //          Tr[0].push_back(cv::Point(tr[0][1].x, tr[0][1].y));
    //          Tr[0].push_back(cv::Point(tr[0][2].x, tr[0][2].y));
    //Tr[0].push_back(cv::Point(tr[0][3].x, tr[0][3].y));// Black or White Tr 2019-6-28
    //          s=1;
...
//printf("TR-shori-END No.=%d \n",m);
            if (idx==3){ //３角形がこの４角形にふくまれる
                ...
                // Four corners of source image
                pts_src.push_back(cv::Point2f(ax[ii], bx[ii]));// 上から右周りの座標
                pts_src.push_back(cv::Point2f(ax[(ii+1)%4], bx[(ii+1)%4]));
                pts_src.push_back(cv::Point2f(ax[(ii+2)%4], bx[(ii+2)%4]));
                pts_src.push_back(cv::Point2f(ax[(ii+3)%4], bx[(ii+3)%4]));

                pts_dst.push_back(cv::Point2f(0, 0));
                pts_dst.push_back(cv::Point2f(250, 0));
                pts_dst.push_back(cv::Point2f(250, 250));
                pts_dst.push_back(cv::Point2f(0, 250));
